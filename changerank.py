...
async def get_top_k(game_id: str, request: Request, k: int =3 ): #if k is not provided, default to 3
    ...
    score_sorted_list = sorted(games[game_id].items(), key=lambda x: x[1]["user_score"], reverse=True)
    if k >= len(games[game_id]):
        message = f"Only {len(games[game_id])} scores available for game_id {game_id}"
...
async def get_game_statistics(game_id: str, request: Request):
    ...
    if game_id not in games:
        return {"message": f"No scores found for game_id {game_id}"}
    total_users = len(games[game_id])
    scores = [user["user_score"] for user in games[game_id].values()] #list comprehension to get all scores
