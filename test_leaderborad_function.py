...
import statistics
def create_or_update_score(entry: ScoreEntry, games_ids):
    if entry.game_id not in games_ids: 
...
def get_top_k(game_id: str,games_ids, k: int =3): #if k is not provided, default to 3
...
def get_user_rank(game_id: str, user_id: str, games_ids):    
    if game_id not in games_ids:
        return {"message": f"No scores found for game_id {game_id}"}
...
def get_game_statistics(game_id: str, games_ids):
    if game_id not in games_ids:
        return {"message": f"No scores found for game_id {game_id}"}
